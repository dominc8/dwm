...
#include <X11/XF86keysym.h>         /* Handling laptop's power off keys etc */
#include <X11/Xatom.h>
#include <X11/Xlib.h>
...
loadxrdb()
{
  ...
  fprintf(stderr, "display is%s null\n", display ? " not" : " ");

  if (display != NULL) {
    resm = XResourceManagerString(display);
    fprintf(stderr, "resm is%s null\n", resm ? " not" : " ");

    if (resm != NULL) {
      fprintf(stderr, "resm: %sl\n", resm);
      xrdb = XrmGetStringDatabase(resm);

      fprintf(stderr, "xrdb is%s null\n", xrdb ? " not" : " ");
      if (xrdb != NULL) {
      ...
      fprintf(stderr, "normbordercolor: %s\n", normbordercolor);
      fprintf(stderr, "normbgcolor: %s\n", normbgcolor);
      fprintf(stderr, "normfgcolor: %s\n", normfgcolor);
      fprintf(stderr, "selbordercolor: %s\n", selbordercolor);
      fprintf(stderr, "selbgcolor: %s\n", selbgcolor);
      fprintf(stderr, "selfgcolor: %s\n", selfgcolor);
    }
  }
